...
def check_profile():
    ...
    print(f"Checking profile for username : {username}")

    # user_profile 테이블에서 해당 username 존재하는지 확인
    query = f"SELECT COUNT(*) AS profile_count FROM user_profile WHERE username = '{username}'"
    result = dbHandler.loadData(query)
    profile_exists = bool(result.iloc[0]['profile_count'] > 0)
...
def save_userprofile():
    ...
    food_codes = []
    food_code_names = []
    for food_code in preferredCategories:
    ...
    # DB에 저장
    dbHandler.saveUserProfile(username, height, weight, age, gender, food_codes_str, food_code_names_str)
...
def recommend():
    ...
    userData = dbHandler.loadData(f"SELECT * FROM user_profile WHERE username = '{username}'")
    ...
    # 사용자 프로필 생성 전에 preferredCategories 값을 쉼표로 구분된 리스트로 변환
    preferredCategories = userData['food_code_name'].iloc[0].split(',')
...
def submit_rating():
    ...
    # 평점 데이터를 저장
    dbHandler.saveFeedback(username, feedback_data)
